...
And(/^I have entered PCN and VRN numbers for the Informal Representation$/) do
  @parking_page.pcn.set "AF72000653"
...
And(/^a PCN status code allows a make a representation to the council$/) do
  wait_for_an_element_to_appear
  find_element_by_text(@parking_page.pcn_challenge, "Make a representation to the council").click
end

And(/^I have selected to challenge a PCN$/) do
  expect(@parking_page).to have_terms_and_conditions_text_para1
  expect(@parking_page).to have_terms_and_conditions_text_para2
  expect(@parking_page).to have_terms_and_conditions_text_para3
  expect(@parking_page).to have_terms_and_conditions_text_para4
  expect(@parking_page).to have_terms_and_conditions_text_para5
  wait_for_an_element_to_appear
  find_element_by_text(@parking_page.log_represntation_btn, "Log a challenge using the online form").click
end
...
And(/^I have selected to log a PCN represntation using online form$/) do
  ...
  expect(@parking_page).to have_terms_and_conditions_text_para5
  wait_for_an_element_to_appear
  find_element_by_text(@parking_page.log_represntation_btn, "Log a representation using the online form").click
end
...
And(/^I attach proof of permission from the owner$/) do
  wait_for_an_element_to_appear
  sleep 2
  page.driver.browser.all(:xpath, '//input[@name="PcnRepresentationReasonCustomViewModel.VehiclePermission"]')[0].send_keys('/assets/reason.jpg')
  sleep 2
